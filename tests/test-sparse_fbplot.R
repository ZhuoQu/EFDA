# tests/test-sparse_fbplot.R
library(testthat)
context("Testing the sparse functional boxplot function")

test_that("The result of the sparse functional boxplot is a list", {
  ## 1. Univariate complete functional data
  library(fda)
  library(roahd)
   canada_temp <- t(CanadianWeather$monthlyTemp)
  result1 <- sparse_fbplot(fit = canada_temp,
                sparse = canada_temp,
  ...
  ##result2 <- sparse_fbplot(fit = fit_cd4, sparse = cd4,
  ##                         time_index = NULL, depth = depth_cd4)
  ...
  ## 3. Bivariate complete functional data
   ...
   result3 <- sparse_fbplot(fit = true_canada, sparse = true_canada, time_index = NULL, depth = as.numeric(mfd_canada$MFDdepthZ))
  ...
  ##  Replace values in the full matrix with NA where the mask is TRUE
   ...
   ##result4 <- sparse_fbplot(fit = fit_canada, sparse = sparse_canada,
