## Description of fbplot_outlier.R.
## add the outliers in an existing plot
...
fbplot_outlier <- function(two_stage = TRUE,
  ...
  for (nums in out_index) {
    ...
    co <- ifelse(nums %in% dir_outlier_index, outliercolor.dir, outliercolor.fb)
    sparse_outindex <- which(is.na(outl_sparse)) ## index where there are observations in the median curves
